    ...
    <div className="pt-28 pb-20 heritage-section min-h-screen">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h1 className="text-3xl sm:text-4xl font-bold text-brand-900 mb-4">{t("title")}</h1>
          <p className="text-brand-700/65">{t("subtitle")}</p>
        </div>

        <div className="grid sm:grid-cols-2 gap-6">
          {contacts.map(({ icon: Icon, label, value, dir }) => (
            <div key={label} className="heritage-card rounded-xl p-6 card-hover">
              <div className="w-12 h-12 rounded-xl gradient-bg flex items-center justify-center mb-4">
                <Icon className="w-6 h-6 text-white" />
              </div>
              <h3 className="text-sm font-medium text-brand-600 mb-1">{label}</h3>
              <p className="text-brand-900 font-semibold" dir={dir}>{value}</p>
            </div>
          ))}
        </div>
      </div>
